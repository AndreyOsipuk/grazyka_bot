...
⚠⚠⚠️ <b>Внимание!</b> Вы должны написать приветственную анкету (имя, пол, возраст, город, фото или мем 18+) в течение ${pluralizeMinutes(TIME_LIMIT_MINUTES)} минуты после вступления, иначе будете забанены. ⚠⚠⚠
...
bot.action(/^(approve|reject)_(\d+)$/, async (ctx) => {
  ...
  if (action === "approve") {
    ...
    const successMsg = [
      ...
      `• После вступления напишите любое сообщение в группе в течение ${pluralizeMinutes(TIME_LIMIT_MINUTES)} минут!`,
  ...
  for (const member of ctx.message.new_chat_members) {
    ...
    const sent = await ctx.replyWithHTML(
      [
        ...
        "",
        `⚠️ <b>Напоминание:</b> Напишите вашу анкету (имя, пол, возраст, город, фото или мем 18+) в течение ${pluralizeMinutes(TIME_LIMIT_MINUTES)} минут.`,
        "",
        "⏰ Время пошло!",
      ].join("\n"),
    );
    ...
    if (!messageHasPhoto(ctx.message)) {
      await ctx.replyWithHTML(
        [
          ...
          `Отправьте фото, иначе вы будете забанены через ${pluralizeMinutes(TIME_LIMIT_MINUTES)} минут после вступления.`,
        ].join("\n"),
      );
