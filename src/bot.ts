  ...
  for (const member of ctx.message.new_chat_members) {
    ...
    const sent = await ctx.replyWithHTML(
      [
        ...
        "",
        `⚠️ <b>Напоминание:</b> Напишите вашу анкету (имя, пол, возраст, город, фото или мем 18+) в течение ${TIME_LIMIT_MINUTES} минут.`,
        "",
        "⏰ Время пошло!",
      ].join("\n"),
    );
    ...
    if (!messageHasPhoto(ctx.message)) {
      await ctx.replyWithHTML(
        [
          ...
          `Отправьте фото, иначе вы будете забанены через ${TIME_LIMIT_MINUTES} минут после вступления.`,
        ].join("\n"),
      );
